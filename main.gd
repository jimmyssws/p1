...
func _redistribute_roles():
	...
	var pres_node = get_node(str(president_id))
...
# 2. KURAL: Eğer başka oyuncu varsa, birini rastgele SUİKASTÇI yap
	if players_to_assign.size() > 0:
		# Kalan oyunculardan rastgele birini seçiyoruz
		var assassin_id = players_to_assign[randi() % players_to_assign.size()]
		var ass_node = get_node(str(assassin_id))
	...
	for guard_id in players_to_assign:
		var guard_node = get_node(str(guard_id))
...
func _spawn_npcs(amount):
	for i in range(amount):
		...
		# Ağda karışmamaları için her birine özel bir isim veriyoruz (Örn: NPC_0, NPC_1...)
		npc.name = "NPC_" + str(i) 
		
		npc.position = Vector3(randf_range(-20, 20), 2, randf_range(-20, 20))
		
		# Godot'un bizden istediği o sihirli "true" kelimesini ekliyoruz
		add_child(npc, true)
